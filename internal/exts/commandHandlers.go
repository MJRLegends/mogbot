...
import (
	"fmt"
	router2 "github.com/captainmog/mogbot/pkg/router"
	"log"
	"time"
...
var CommandHandlers = map[string]router2.CommandHandler{
	"ping": func(c *router2.Context) {
	...
	"move": func(c *router2.Context) {
		if !router2.IsAdmin(c.Session, c.GuildID, c.Member) {
		...
		if ok, o := router2.FindOption("from", c.Data.Options); ok {
			from = o.ChannelValue(c.Session)
		}
		var to *discordgo.Channel
		if ok, o := router2.FindOption("to", c.Data.Options); ok {
	...
	//	})
	//},
	"ban": func(c *router2.Context) {
		if !router2.CanBan(c.Session, c.GuildID, c.Member) {
			err := c.InteractionRespond(c.Interaction, &discordgo.InteractionResponse{
				Type: discordgo.InteractionResponseChannelMessageWithSource,
				Data: &discordgo.InteractionApplicationCommandResponseData{
		...
		var reason string = ""
		var days int = 0
		if found, o := router2.FindOption("user", c.Data.Options); found {
			user = o.UserValue(c.Session)
		}
		if found, o := router2.FindOption("reason", c.Data.Options); found {
			reason = o.StringValue()
		}
		if found, o := router2.FindOption("days", c.Data.Options); found {
			days = int(o.IntValue())
		}
	...
	"embed": func(c *router2.Context) {
		...
		case "edit":
			found, o := router2.FindOption("channel", c.Data.Options[0].Options)
			if !found {
				log.Printf("Unable to find channel option")
				return
			}
			channelID := o.ChannelValue(c.Session).ID
			found, o = router2.FindOption("messageid", c.Data.Options[0].Options)
			if !found {
			...
			if found, o := router2.FindOption("title", c.Data.Options[0].Options); found {
				title = fmt.Sprintf("%v", o.Value)
			}
			if found, o := router2.FindOption("description", c.Data.Options[0].Options); found {
				desc = fmt.Sprintf("%v", o.Value)
			}
			if found, o := router2.FindOption("color", c.Data.Options[0].Options); found {
				color = int(o.IntValue())
			}
			if found, o := router2.FindOption("author", c.Data.Options[0].Options); found {
				u := o.UserValue(c.Session)
				author.Name = u.String()
				author.IconURL = u.AvatarURL("")
			}
			if found, o := router2.FindOption("thumbnail", c.Data.Options[0].Options); found {
				thumbnail.URL = o.StringValue()
			}
			if found, o := router2.FindOption("footertext", c.Data.Options[0].Options); found {
				footer.Text = o.StringValue()
			}
			if found, o := router2.FindOption("footericon", c.Data.Options[0].Options); found {
			...
			if found, o := router2.FindOption("timestamp", c.Data.Options[0].Options); found {
				if o.BoolValue() {
					timestamp = time.Now().Format(time.RFC3339)
				}
			}
		...
		case "append":
			...
			found, o = router2.FindOption("text", c.Data.Options[0].Options)
			if !found {
