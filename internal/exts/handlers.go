...
var Handlers = []mogbot.Handler{
	...
	func(b *mogbot.Bot) interface{} {
		return func(s *discordgo.Session, g *discordgo.GuildCreate) {
			for _, r := range g.Roles {
				if err := b.DB.AddRole(*mogbot.NewRole(r)); err != nil {
					continue
				}
			}
	...
	func(b *mogbot.Bot) interface{} {
		return func(s *discordgo.Session, m *discordgo.MessageCreate) {
			...
			guild, err := s.Guild(mogbot.GuildID)
			if err != nil {
			...
			s.ChannelMessageSendEmbed(mogbot.ModMailLogID, logNewTicket)
			s.ChannelMessageSendEmbed(m.ChannelID, reply)
			data := discordgo.GuildChannelCreateData{
				Name:     chName,
				ParentID: mogbot.ModMailCatID,
			}
			ch, err := s.GuildChannelCreateComplex(mogbot.GuildID, data)
			if err != nil {
				log.Print(err)
				return
			}
